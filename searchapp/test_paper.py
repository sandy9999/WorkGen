...
def form_test_paper_dictionary(subject, chapters, subject_breakup):
    ...
    for question_type in subject_breakup:
        ...
        total_list = Questions.objects.filter(question_type=q_type, question_weightage=weightage,
...
def generate_customized_paper(subject, chapters, subject_breakup, data, mentor, token):
    ...
    it returns a list of dicts of form==>{stud_name_1: [{'question_type':'','question':'','attempt':''},{},{}],
                                        stud_name_2: [{'question_type':'','question':'','attempt':''},{},{}]}
    """
    final_dict = {}
    ...
    GeneratedCustomizedPaper.objects.filter(mentor__username=mentor, token=token).update(file_path=filepath,
                                                                                         is_ready=True)
